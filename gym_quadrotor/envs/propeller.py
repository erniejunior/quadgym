...
class Propeller(object):
    ...
    def get_dynamics(self, speed, copter_status):
        """ Gets the dynamics (force and momentum)
        ...
        trafo    = Quaternion(trafo[0], trafo[1], trafo[2]).rotation_matrix()
        # convert axis from copter to world frame
        axis     = np.dot(trafo, self._axis)
        pos      = np.dot(trafo, self._position)

        F        = force(self, axis, velocity, ang_vel)
        M        = momentum(self, axis, velocity, ang_vel)
        M       += np.cross(pos, F)
        return F, M
